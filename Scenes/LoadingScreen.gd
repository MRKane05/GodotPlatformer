...
func reload_scene(current_scene) -> void:
	...
	var loader = ResourceLoader.load_interactive(Global.target_scene)
	if loader == null:
		print("Resource loader unable to load the resource at path: " + Global.target_scene)
		return
	...
	while true:
		...
		if err == ERR_FILE_EOF:
			...
			if Global.target_door:
				resource_instance.call_deferred("set_player_door", Global.target_door)
